<template>
  <div class="component-status">
    <ul class="status-list">
      <li>
        <EIcon name="ready" fill="#7cb518" size="small" />
        <p>Ready</p>
      </li>
      <li>
        <EIcon name="review" :fill="tokens.color_ucla_gold.value" size="small" />
        <p>Under review</p>
      </li>
      <li>
        <EIcon name="deprecated" :fill="tokens.color_vermilion.value" size="small" />
        <p>Deprecated</p>
      </li>
      <li>
        <EIcon name="prototype" :fill="tokens.color_bleu_de_france.value" size="small" />
        <p>Prototype</p>
      </li>
      <li>
        <span>—</span>
    ...
    <table>
      <thead>
        <tr>
          ...
          <th v-if="show === 'tokens'">Token Component Name</th>
          <th>Released in</th>
          <th>Status</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="(component, index) in components" :key="index" class="component">
          ...
          <td v-if="component.status">
            <EIcon v-if="component.status === 'ready'" name="ready" fill="#7cb518" size="small" />
            <EIcon
...
export default {
  ...
  props: {
    show: {
      ...
      default: "all",
      validator: value => {
        return value.match(/(all|patterns|templates|elements|tokens)/)
      },
    },
  },
  methods: {
    getComponents: function() {
      ...
      if (this.show === "all") {
        contexts = [
          ...
          require.context("../tokens/", true, /\.vue$/),
        ]
